...
export default function AdminArticles() {
  ...
  // 🎨 WARNA UNTUK CARD ARTIKEL
  const kategoriColors = {
    Burnout: { bg: "#FFF1F2", color: "#9F1239", emoji: "😤", gradient: "linear-gradient(135deg, #FECDD3, #FDA4AF)" },
    Cemas: { bg: "#F5F3FF", color: "#6D28D9", emoji: "😰", gradient: "linear-gradient(135deg, #E9D5FF, #D8B4FE)" },
    Sedih: { bg: "#EFF6FF", color: "#1E40AF", emoji: "😢", gradient: "linear-gradient(135deg, #BFDBFE, #93C5FD)" },
    Netral: { bg: "#F0FDF4", color: "#166534", emoji: "😌", gradient: "linear-gradient(135deg, #DCFCE7, #BBF7D0)" },
    Krisis: { bg: "#FFF1F2", color: "#9F1239", emoji: "🆘", gradient: "linear-gradient(135deg, #FECDD3, #FDA4AF)" },
  };
      ...
      setStatsModalData({
        ...
        content: (
          <div>
            ...
            <p style={{ fontSize: 48, fontWeight: 700, textAlign: 'center', color: '#8B5CF6' }}>{kategoriStats.total}</p>
...
<span style={styles.navLogoText}>RuangTenang</span>
      ...
      <div style={styles.container}>
        ...
        <div style={styles.statsGrid}>
          {/* Total Card - Soft Purple (SAMA DENGAN DATA JURNAL) */}
          <div 
          ...
          {/* Burnout Card - Soft Rose (Pink) */}
          <div 
            style={{ ...styles.statCard, background: kategoriColors.Burnout.gradient, cursor: 'pointer', border: 'none', color: '#9F1239' }}
            onClick={() => handleStatsCardClick('kategori', 'Burnout')}
            onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'}
            onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}
          >
            <div style={styles.statIcon}>{kategoriColors.Burnout.emoji}</div>
            <div style={{ ...styles.statNum, color: '#9F1239' }}>{kategoriStats.Burnout}</div>
            <div style={{ ...styles.statLabel, color: '#9F1239' }}>Burnout</div>
            <div style={{ ...styles.clickHint, color: '#9F1239' }}>✨ Klik untuk detail</div>
          </div>

          {/* Cemas Card - Soft Lavender */}
          <div 
            style={{ ...styles.statCard, background: kategoriColors.Cemas.gradient, cursor: 'pointer', border: 'none', color: '#6D28D9' }}
            onClick={() => handleStatsCardClick('kategori', 'Cemas')}
            onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'}
            onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}
          >
            <div style={styles.statIcon}>{kategoriColors.Cemas.emoji}</div>
            <div style={{ ...styles.statNum, color: '#6D28D9' }}>{kategoriStats.Cemas}</div>
            <div style={{ ...styles.statLabel, color: '#6D28D9' }}>Cemas</div>
            <div style={{ ...styles.clickHint, color: '#6D28D9' }}>✨ Klik untuk detail</div>
          </div>

          {/* Sedih Card - Soft Blue */}
          <div 
            style={{ ...styles.statCard, background: kategoriColors.Sedih.gradient, cursor: 'pointer', border: 'none', color: '#1E40AF' }}
            onClick={() => handleStatsCardClick('kategori', 'Sedih')}
            onMouseEnter={(e) => e.currentTarget.style.transform = 'translateY(-4px)'}
            onMouseLeave={(e) => e.currentTarget.style.transform = 'translateY(0)'}
          >
            <div style={styles.statIcon}>{kategoriColors.Sedih.emoji}</div>
            <div style={{ ...styles.statNum, color: '#1E40AF' }}>{kategoriStats.Sedih}</div>
            <div style={{ ...styles.statLabel, color: '#1E40AF' }}>Sedih</div>
            <div style={{ ...styles.clickHint, color: '#1E40AF' }}>✨ Klik untuk detail</div>
          </div>
        </div>
...
const styles = {
  ...
  formCard: { background: "white", borderRadius: 24, border: "1px solid #E2E8F0", marginBottom: 32, overflow: "hidden" },
  formHeader: { padding: "16px 24px", borderBottom: "1px solid #E2E8F0", background: "#F8FAFC" },
  formTitle: { fontSize: 16, fontWeight: 600, color: "#0F172A", margin: 0 },
  formBody: { padding: "24px" },
  input: { width: "100%", padding: "12px 16px", border: "1px solid #E2E8F0", borderRadius: 12, fontSize: 14, marginBottom: 16, outline: "none", transition: "all 0.2s ease" },
  select: { width: "100%", padding: "12px 16px", border: "1px solid #E2E8F0", borderRadius: 12, fontSize: 14, marginBottom: 16, outline: "none", background: "white", cursor: "pointer" },
  textarea: { width: "100%", padding: "12px 16px", border: "1px solid #E2E8F0", borderRadius: 12, fontSize: 14, marginBottom: 16, outline: "none", fontFamily: "monospace", resize: "vertical" },
  addBtn: { padding: "12px 24px", background: "linear-gradient(135deg, #8B5CF6, #7C3AED)", color: "white", border: "none", borderRadius: 40, cursor: "pointer", fontSize: 14, fontWeight: 600, transition: "all 0.2s ease" },
  ...
  searchInput: { width: "100%", padding: "12px 14px 12px 38px", border: "1px solid #E2E8F0", borderRadius: 48, fontSize: 14, background: "white", outline: "none", transition: "all 0.2s ease" },
  clearBtn: { position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "#F1F5F9", border: "none", borderRadius: 20, padding: "4px 10px", fontSize: 12, cursor: "pointer", color: "#64748B" },
  card: { background: "white", borderRadius: 24, border: "1px solid #E2E8F0", boxShadow: "0 4px 12px rgba(0,0,0,0.02)", overflow: "hidden" },
  table: { width: "100%", borderCollapse: "collapse" },
  theadRow: { background: "#F8FAFC" },
  th: { textAlign: "left", padding: "16px 20px", fontSize: 13, fontWeight: 600, color: "#475569", borderBottom: "1px solid #E2E8F0" },
  thNo: { textAlign: "center", padding: "16px 20px", fontSize: 13, fontWeight: 600, color: "#475569", borderBottom: "1px solid #E2E8F0", width: 70 },
  thAksi: { textAlign: "center", padding: "16px 20px", fontSize: 13, fontWeight: 600, color: "#475569", borderBottom: "1px solid #E2E8F0", width: 100 },
  td: { padding: "16px 20px", fontSize: 14, color: "#1E293B", borderBottom: "1px solid #F1F5F9" },
  tdNo: { textAlign: "center", padding: "16px 20px", fontSize: 14, color: "#64748B", borderBottom: "1px solid #F1F5F9" },
  tdAksi: { textAlign: "center", padding: "16px 20px", borderBottom: "1px solid #F1F5F9" },
  tr: { transition: "background 0.2s ease" },
  articleCell: { display: "flex", alignItems: "center", gap: 12, cursor: "pointer" },
  thumbnail: { width: 60, height: 50, objectFit: "cover", borderRadius: 10 },
  thumbnailPlaceholder: { width: 60, height: 50, borderRadius: 10, background: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 },
  articleTitle: { fontWeight: 600, marginBottom: 2, color: "#0F172A" },
  articleAuthor: { fontSize: 12, color: "#94A3B8" },
  badge: { padding: "4px 14px", borderRadius: 30, fontSize: 12, fontWeight: 600, display: "inline-block" },
  deleteBtn: { padding: "6px 16px", fontSize: 12, borderRadius: 30, border: "1px solid #FECACA", background: "#FEF2F2", color: "#DC2626", cursor: "pointer", transition: "all 0.2s ease" },
  ...
  emptyText: { fontSize: 16, fontWeight: 500, color: "#1E293B", marginBottom: 8 },
  ...
  modalContent: { background: "white", borderRadius: 28, maxWidth: 700, width: "90%", maxHeight: "80vh", overflow: "auto", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" },
  ...
  modalMeta: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, paddingBottom: 12, borderBottom: "1px solid #E2E8F0" },
  modalKategori: { padding: "4px 12px", borderRadius: 20, fontSize: 12, background: "#F1F5F9", color: "#475569" },
  modalContentArtikel: { fontSize: 14, lineHeight: 1.6, color: "#334155" },
};
